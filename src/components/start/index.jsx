...
export const DesktopApp = () => {
  const desktopApps = useSelector((state) => state.desktop.apps);
  const desktopSort = useSelector((state) => state.desktop.sort);
  const desktopHide = useSelector((state) => state.desktop.hide);
  const desktopSize = useSelector((state) => state.desktop.size);

  const sortedApps = useMemo(() => {
    var tmpApps = [...desktopApps];
    if (desktopSort == "name") {
          ...
          return (
            ...
            <div key={i} className="dskApp" tabIndex={0}>
              <Icon
                ...
                pr
                width={Math.round(desktopSize * 36)}
                menu="app"
              />
